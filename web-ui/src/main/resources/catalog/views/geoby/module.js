(function () {
  goog.provide("gn_search_geoby");
  goog.require("gn_search_default");

  var module = angular.module("gn_search_geoby", ["gn_search_default"]);

  // Add a custom geoby locale file
  module.config([
    "$LOCALES",
    function ($LOCALES) {
      $LOCALES.push("geoby");
  ...
  module.controller("gnsGeoby", [
    ...
    // Для логотипа
      $scope.isNipdPortal = function() {
        var currentPortalId = $scope.info['node/id'] || $scope.info['system/site/siteId'];
        return currentPortalId === 'srv';
      };
    }
  ]);
