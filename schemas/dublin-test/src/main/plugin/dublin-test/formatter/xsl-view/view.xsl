...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
 ...
 <!-- tr is defined at  core-geonetwork/services/src/main/java/org/fao/geonet/api/records/formatters/SchemaLocalizations.java -->
  ...
  <xsl:variable name="metadata"
                select="/root/simpledt"/>


  <!-- Specific schema rendering -->
  <xsl:template mode="getMetadataTitle" match="simpledt">
    <xsl:value-of select="dc:title"/>
  </xsl:template>

  <xsl:template mode="getMetadataHierarchyLevel" match="simpledt">
    <xsl:value-of select="'dataset'"/>
  </xsl:template>

  <xsl:template mode="getMetadataAbstract" match="simpledt">
    <xsl:call-template name="addLineBreaksAndHyperlinks">
      <xsl:with-param name="txt" select="dc:description"/>
    </xsl:call-template>
  </xsl:template>

  <xsl:template mode="getMetadataHeader" match="simpledt">
  ...
  <xsl:template mode="getOverviews" match="simpledt">
  ...
  <xsl:template mode="getExtent" match="simpledt">
  ...
  <xsl:template mode="getTags" match="simpledt">
  ...
  <!-- Traverse the tree -->
  <xsl:template mode="render-field" match="simpledt">
    <xsl:apply-templates mode="render-field"/>
  </xsl:template>
