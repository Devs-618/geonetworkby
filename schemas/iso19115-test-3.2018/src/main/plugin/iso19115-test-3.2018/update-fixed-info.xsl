  ...
  <xsl:template match="mdb:MD_Metadata">
    <xsl:copy copy-namespaces="no">
      ...
      <xsl:if test="/root/env/createDate != '' and
        ...
        <mdb:dateInfo>
          <xsl:copy-of select="gn-fn-iso19115-test-3.2018:write-date-or-dateTime(/root/env/createDate, 'creation')"/>
        </mdb:dateInfo>
      </xsl:if>
      <xsl:if test="/root/env/changeDate != '' and not($isRevisionDateAvailable)">
        <mdb:dateInfo>
          <xsl:copy-of select="gn-fn-iso19115-test-3.2018:write-date-or-dateTime(/root/env/changeDate, 'revision')"/>
        </mdb:dateInfo>
      </xsl:if>
  ...
  <xsl:template match="*[gco:CharacterString|gcx:Anchor|lan:PT_FreeText]">
    <xsl:copy>
      ...
      <xsl:variable name="excluded"
                    select="gn-fn-iso19115-test-3.2018:isNotMultilingualField(., $editorConfig)"/>
